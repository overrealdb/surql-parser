...
use crate::upstream::syn::lexer::unicode::{byte, chars};
...
impl Lexer<'_> {
	...
	fn eat_string_like(&mut self, is_double: bool) -> Result<(), SyntaxError> {
		...
		loop {
			let Some(byte) = self.reader.next() else {
				bail!(
					"Unexpected end of file, expected string to end.", @ start_span =>
			...
			match byte {
				...
				b'\\' => {
					let Some(x) = self.reader.next() else {
						bail!(
							"Unexpected end of file, expected string to end.", @
							start_span => "String starting here."
						);
					};
					if !x.is_ascii() {
	...
	pub(super) fn eat_multi_line_comment(&mut self) -> Result<(), SyntaxError> {
		...
		loop {
			let Some(byte) = self.reader.next() else {
				bail!(
					"Unexpected end of file, expected multi-line comment to end.", @
					start_span => "Comment starts here."
				);
			};
			if let b'*' = byte {
		...
		let kind = match byte {
			...
			b'&' => match self.reader.peek() {
				...
				_ => {
					let error = syntax_error!(
						"Invalid token `&`, single `&` are not a valid token, did you mean `&&`?",@
						self.current_span()
					);
			...
			b'+' => match self.reader.peek() {
				...
				Some(b'?') => {
					...
					match self.reader.peek() {
						...
						_ => {
							let error = syntax_error!(
								"Invalid token `+?` did you maybe mean `+?=` ?", @ self
								.current_span()
							);
							return self.invalid_token(error);
						}
			...
			x => {
				let err = syntax_error!(
					"Invalid token `{}`", x as char, @ self.current_span()
				);
