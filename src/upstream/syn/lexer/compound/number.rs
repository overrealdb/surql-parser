use crate::compat::val::DecimalExt;
use crate::compat::val::duration::{
...
use crate::upstream::syn::parser::ParseResult;
use crate::upstream::syn::token::{Span, Token, TokenKind, t};
...
pub fn numeric_kind(lexer: &mut Lexer, start: Token) -> Result<NumericKind, SyntaxError> {
	match start.kind {
		...
		x => {
			bail!(
				"Unexpected token `{x}`, expected a numeric value, either a duration or number",@
				start.span
			)
		}
	}
...
pub fn number_kind(lexer: &mut Lexer, start: Token) -> Result<NumberKind, SyntaxError> {
	...
	if has_ident_after(lexer) {
		...
		bail!(
			"Invalid token, found unexpected character `{char}` after number token", @
			lexer.current_span()
		)
	}
	Ok(kind)
	...
	match kind {
		...
		NumberKind::Float => {
				...
				|| number_str.as_bytes()[0] == b'I'
			{
				...
				number_str.parse().map(Numeric::Float).map_err(|e| {
					syntax_error!(
						"Failed to parse number: {e}", @ lexer.current_span()
					)
				})
			}
		}
		NumberKind::Decimal => {
			...
			let decimal = if number_str.contains(['e', 'E']) {
				Decimal::from_scientific(number_str).map_err(|e| {
					syntax_error!(
						"Failed to parser decimal: {e}", @ lexer.current_span()
					)
				})?
			} else {
				crate::compat::decimal_from_str_normalized(number_str).map_err(|e| {
					syntax_error!(
						"Failed to parser decimal: {e}", @ lexer.current_span()
					)
				})?
	...
	I: FromStr<Err = ParseIntError>,
{
	...
	if has_ident_after(lexer) {
		...
		let char = lexer.reader.convert_to_char(char)?;
		bail!(
			"Invalid token, found unexpected character `{char} after integer token", @
			lexer.current_span()
		)
	}
	...
	if peek == Some(b'e') || peek == Some(b'E') {
		bail!(
			"Unexpected character `{}` only integers are allowed here", peek
			.expect("validated input") as char, @ lexer.current_span()
		)
	}
	let span = lexer.current_span();
	...
	F: Float,
{
	...
	if has_ident_after(lexer) {
		...
		let char = lexer.reader.convert_to_char(char)?;
		bail!(
			"Invalid token, found invalid character `{char}` after number token", @ lexer
			.current_span()
		)
	}
...
pub fn duration(lexer: &mut Lexer, start: Token) -> Result<Duration, SyntaxError> {
	...
	loop {
		...
		let numeric_value: u64 = numeric_string.parse().map_err(|e| {
			syntax_error!(
				"Invalid token, failed to parse duration digits: {e}",@ lexer
				.current_span()
			)
		})?;
		let addition = match suffix {
			...
			DurationSuffix::Minute => {
				let minutes = numeric_value
					.checked_mul(SECONDS_PER_MINUTE)
					.ok_or_else(|| {
						syntax_error!(
							"Invalid duration, value overflowed maximum allowed value", @
							lexer.current_span()
						)
					})?;
		...
		duration = duration.checked_add(addition).ok_or_else(|| {
			syntax_error!(
				"Invalid duration, value overflowed maximum allowed value", @ lexer
				.current_span()
			)
		})?;
...
fn lex_duration_suffix(lexer: &mut Lexer) -> Result<DurationSuffix, SyntaxError> {
	let suffix = match lexer.reader.next() {
		...
		Some(0xC2) => {
			if !lexer.eat(0xB5) {
				...
				bail!(
					"Invalid duration token, expected a duration suffix found `{char}`",@
					lexer.current_span()
				)
			}
			lexer.expect('s')?;
		...
		None => {
			bail!(
				"Unexpected end of file, expected a duration suffix",@ lexer
				.current_span()
			)
		}
	};
	if has_ident_after(lexer) {
		...
		let char = lexer.reader.convert_to_char(char)?;
		bail!(
			"Invalid token, found invalid character `{char}` after duration suffix", @
			lexer.current_span()
		)
	}
	Ok(suffix)
...
fn eat_digits1(lexer: &mut Lexer, start: u32) -> Result<(), SyntaxError> {
	match lexer.reader.peek() {
		...
		Some(x) => {
			let char = lexer.reader.convert_to_char(x)?;
			bail!(
				"Invalid number token, expected a digit, found: {char}", @ lexer
				.span_since(start)
			);
		}
		None => {
			bail!(
				"Unexpected end of file, expected a number token digit", @ lexer
				.span_since(start)
			);
		}
	}
	...
	if lexer.reader.eat(b'I') {
		for b in b"nfinity" {
			match lexer.reader.next() {
				...
				Some(x) => {
					bail!(
						"Invalid number token, expected `{}` found: {x}",* b as char, @
						lexer.span_since(start)
					)
				}
				None => {
					bail!(
