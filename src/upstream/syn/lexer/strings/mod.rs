	...
	pub(super) fn lex_common_escape_sequence(
		...
		match c {
			...
			_ => {
				let span = reader.span_since(before).as_within(span);
				bail!(
					"Invalid escape sequence", @ span => "not a valid escape character"
				)
			}
		}
		Ok(())
		...
		if reader.eat(b'{') {
			...
			for _ in 0..6 {
				match reader.peek() {
					...
					_ => {
						...
						bail!(
							"Invalid escape sequence, expected `}}` or hexadecimal character.",
							@ span => "Unexpected character"
						)
					}
				}
			}
			if !reader.eat(b'}') {
				...
				if n.map(|x| x.is_ascii_hexdigit()).unwrap_or(false) {
					bail!(
						"Invalid escape sequence, expected `}}` character.", @ span =>
						"Too many hex-digits"
					)
				} else {
					bail!(
			...
			char::from_u32(accum).ok_or_else(|| {
				let span = reader.span_since(before).as_within(span);
				syntax_error!(
					"Invalid escape sequence, unicode escape character is not a valid unicode character.",
					@ span => "Not a valid character code"
				)
			})
		} else {
			let mut accum = 0;
			for _ in 0..4 {
				match reader.next() {
					...
					_ => {
						...
						bail!(
							"String contains invalid escape sequence, expected a hexadecimal character.",
							@ span => "Unexpected character"
						)
					}
				}
			...
			char::from_u32(accum)
                .ok_or_else(|| {
                    let span = reader.span_since(before).as_within(span);
                    syntax_error!(
                        "String contains invalid escape sequence, unicode escape character is not a valid unicode character.",
                        @ span => "Not a valid character code"
                    )
                })
		}
		...
		fn expect_seperator(reader: &mut BytesReader<'_>) -> Result<(), SyntaxError> {
			...
			match reader.next() {
				...
				Some(x) => {
					...
					bail!(
						"Unexpected character `{c}`, expected byte seperator `-`", @ span
					);
				}
				None => {
					let span = reader.span_since(before);
					bail!(
						"Unexpected end of string, expected UUID token to finish", @ span
					);
				}
			}
		}
		...
		while let Some(x) = reader.next() {
			let byte1 = match x {
				...
				x => {
					...
					let span = reader.span_since(before);
					bail!(
						"Unexpected character `{c}`, expected a hexidecimal digit", @
						span
					);
				}
			};
			let Some(x) = reader.next() else {
				...
				bail!(
					"Unexpected end of byte-string, expected a hexidecimal digit", @ span
				);
			};
			let byte2 = match x {
				b'0'..=b'9' => x - b'0',
	...
	pub fn lex_file(str: &str) -> Result<PublicFile, SyntaxError> {
		...
		loop {
			...
			let Some(x) = reader.next() else {
				let span = reader.span_since(reader.offset());
				bail!(
					"Unexpected end of file string, missing bucket seperator `:/`", @
					span
				);
			};
			match x {
				...
				x => {
					...
					let c = reader.convert_to_char(x).expect("lexer validated input");
					bail!(
						"Unexpected character `{c}`, file strings buckets only allow alpha numeric characters and `_`, `-`, and `.`",
						@ span
					);
				}
			}
		...
		while let Some(x) = reader.next() {
			match x {
				...
				x => {
					...
					let c = reader.convert_to_char(x).expect("lexer validated input");
					bail!(
						"Unexpected character `{c}`, file strings key's only allow alpha numeric characters and `_`, `-`, `.`, and `/`",
						@ span
					);
				}
			}
