...
use crate::upstream::sql::operator::{
	BindingPower, BooleanOperator, MatchesOperator, NearestNeighbor,
};
use crate::upstream::sql::{BinaryOperator, Expr, Literal, Part, PostfixOperator, PrefixOperator};
use crate::upstream::syn::error::bail;
use crate::upstream::syn::lexer::compound::Numeric;
...
impl Parser<'_> {
	...
	pub async fn parse_expr_table(&mut self, stk: &mut Stk) -> ParseResult<Expr> {
		...
		let res = enter_query_recursion!(
			this = self => { this.pratt_parse_expr(stk, BindingPower::Base). await }
		);
	...
	pub(super) fn parse_nearest_neighbor(&mut self, token: Token) -> ParseResult<NearestNeighbor> {
		...
		let res = if self.eat(t!(",")) {
			...
			match token.kind {
				...
				_ => {
					bail!(
						"Unexpected token {} expected a distance of an integer", token
						.kind, @ token.span =>
						"The NN operator accepts either a distance or an EF value (integer)"
					)
				}
			}
		} else {
		...
		if !self.eat(t!("|")) || !self.eat_whitespace(t!(">")) {
			bail!(
				"Unexpected token `{}` expected delimiter `|>`", self.peek().kind, @ self
				.recent_span(), @ token.span => "expected this delimiter to close"
			);
		}
		Ok(res)
	}
	...
	async fn parse_infix_op(
			...
			&& BindingPower::for_expr(&lhs) == BindingPower::for_binary_operator(&operator)
		{
			...
			if matches!(
				...
				bail!(
					"Chained range operators has no specified associativity", @ span =>
					"use parens, '()', to specify which operator must be evaluated first"
				)
			} else {
				bail!(
					"Chained relational operators have no defined associativity.", @ span
					=>
					"Use parens, '()', to specify which operator must be evaluated first"
				)
			}
			...
			&& BindingPower::for_expr(&rhs) == BindingPower::for_binary_operator(&operator)
		{
			...
			if matches!(
				...
				bail!(
					"Chained range operators have no defined associativity.", @ span =>
					"Use parens, '()', to specify which operator must be evaluated first"
				)
			} else {
				bail!(
		...
		let op = match token.kind {
			t!(">") => {
				...
				if !lhs_prime && Self::expr_is_range(&lhs) {
					bail!(
						"Chaining range operators has no specified associativity", @
						token.span =>
						"use parens, '()', to specify which operator must be evaluated first"
					)
				}
	...
	pub fn reject_letless_let(expr: &Expr, span: Span) -> ParseResult<()> {
		...
		bail!(
			"Parameter declarations without `let` are deprecated.", @ span =>
			"Replace with `let {} = ...` to keep the previous behavior.", p.to_sql()
		)
	}
}
