...
fn less_then_idiom() {
	...
	"#;
    syn::parse_with(src.as_bytes(), async |parser, stk| parser.parse_query(stk).await)
        .unwrap();
}
#[test]
fn ident_is_field() {
    let mut field = syn::parse_with(
            r#"foo"#.as_bytes(),
            async |parser, stk| parser.parse_query(stk).await,
        )
        .unwrap();
    ...
    assert_eq!(
        exprs,
        sql::TopLevelExpr::Expr(sql::Expr::Idiom(sql::Idiom(vec![sql::Part::Field("foo"
        .to_string())])))
    );
}
#[test]
