...
use crate::upstream::syn::parser::{Parser, ParserSettings};
use crate::compat::types::PublicGeometry;
#[test]
fn parse_index_expression() {
    let value = syn::parse_with(
            "a[1 + 1]".as_bytes(),
    ...
    assert_eq!(
        x.0[1], Part::Value(Expr::Binary { left :
        Box::new(Expr::Literal(Literal::Integer(1))), op : BinaryOperator::Add, right :
        Box::new(Expr::Literal(Literal::Integer(1))), })
    )
}
#[test]
fn parse_coordinate() {
    let coord = syn::parse_with(
            "(1.88, -18.0)".as_bytes(),
            async |parser, stk| { parser.parse_expr_field(stk).await },
        )
        .unwrap();
...
fn parse_numeric_object_key() {
    let v = syn::parse_with(
            "{ 00: 0 }".as_bytes(),
            async |parser, stk| { parser.parse_expr_table(stk).await },
        )
        .unwrap();
...
fn parse_large_depth_object() {
    ...
    let Expr::Literal(Literal::Object(ref object)) = query else { panic!() };
    let mut object = object;
    for _ in 0..999 {
        let Some(Expr::Literal(Literal::Object(new_object))) = object
            .iter()
            .find(|x| x.key == "foo")
            .map(|x| &x.value) else { panic!() };
        object = new_object;
    }
}
#[test]
fn parse_large_depth_record_id() {
    ...
    let mut stack = Stack::new();
    let query = stack.enter(|stk| parser.parse_expr_inherit(stk)).finish().unwrap();
    let Expr::Literal(Literal::RecordId(ref rid)) = query else { panic!() };
    let mut rid = rid;
    for _ in 0..999 {
        let RecordIdKeyLit::Array(ref x) = rid.key else { panic!() };
        let Expr::Literal(Literal::RecordId(ref new_rid)) = x[0] else { panic!() };
        rid = new_rid;
    }
}
#[test]
fn parse_recursive_record_string() {
    let res = syn::parse_with(
        ...
        .unwrap();
    assert_eq!(
        res, Expr::Literal(Literal::RecordId(RecordIdLit { table : "a".to_owned(), key :
        RecordIdKeyLit::Array(vec![Expr::Literal(Literal::RecordId(RecordIdLit { table :
        "b".to_owned(), key : RecordIdKeyLit::Object(vec![ObjectEntry { key : "c"
        .to_owned(), value : Expr::Literal(Literal::RecordId(RecordIdLit { table : "d"
        .to_owned(), key : RecordIdKeyLit::Number(1) })) }]) }))]) }))
    )
}
#[test]
fn parse_record_string_2() {
    ...
    assert_eq!(
        res, Expr::Literal(Literal::RecordId(RecordIdLit { table : "a".to_owned(), key :
        RecordIdKeyLit::Array(vec![Expr::Literal(Literal::String("foo".to_owned()))]) }))
    )
}
#[test]
fn parse_i64() {
...
fn constant_uppercase() {
    ...
    assert_eq!(out, Expr::Constant(Constant::MathPi));
    let out = syn::parse_with(
            r#" MATH::INFINITY "#.as_bytes(),
            async |parser, stk| { parser.parse_expr_field(stk).await },
        )
        .unwrap();
    assert_eq!(out, Expr::Constant(Constant::MathInfinity));
    let out = syn::parse_with(
            r#" MATH::NEG_INFINITY "#.as_bytes(),
            async |parser, stk| { parser.parse_expr_field(stk).await },
        )
        .unwrap();
    assert_eq!(out, Expr::Constant(Constant::MathNegInfinity));
...
fn number_method() {
    let res = syn::parse_with(
            r#" 9.7e-5.sin()"#.as_bytes(),
            async |parser, stk| { parser.parse_expr_field(stk).await },
        )
        .unwrap();
    let expected = Expr::Idiom(
        Idiom(
            vec![
                Part::Start(Expr::Literal(Literal::Float(9.7e-5))), Part::Method("sin"
                .to_string(), vec![]),
            ],
        ),
    );
    ...
    let expected = Expr::Idiom(
        Idiom(
            vec![
                Part::Start(Expr::Literal(Literal::Integer(1))), Part::Method("sin"
                .to_string(), vec![]),
            ],
        ),
    );
