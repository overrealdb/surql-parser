use crate::compat::catalog::{ApiMethod, EventDefinition, EventKind};
use crate::compat::types::PublicDuration;
#[cfg(feature = "surrealism")]
use crate::compat::types::PublicFile;
use crate::upstream::sql::access::AccessDuration;
use crate::upstream::sql::access_type::JwtAccessVerify;
use crate::upstream::sql::base::Base;
use crate::upstream::sql::filter::Filter;
use crate::upstream::sql::index::{Distance, HnswParams, VectorType};
use crate::upstream::sql::kind::KindLiteral;
use crate::upstream::sql::statements::define::config::api::{ApiConfig, Middleware};
use crate::upstream::sql::statements::define::config::defaults::DefaultConfig;
use crate::upstream::sql::statements::define::config::graphql::{GraphQLConfig, TableConfig};
use crate::upstream::sql::statements::define::config::{ConfigInner, graphql};
...
#[cfg(feature = "surrealism")]
use crate::upstream::sql::{ModuleExecutable, SiloExecutable, SurrealismExecutable};
...
use crate::upstream::syn::token::{Token, TokenKind, t};
use reblessive::Stk;
impl Parser<'_> {
	...
	pub async fn parse_define_function(
		...
		loop {
			...
			let param = self.next_token_value::<Param>()?.to_string();
			expected!(self, t!(":"));
	...
	pub async fn parse_define_module(
		...
		bail!(
			"Surrealism modules are not supported in WASM environments", @ self
			.last_span() =>
			"Use of `DEFINE MODULE` is not supported in WASM environments"
		)
	}
	#[cfg(feature = "surrealism")]
	pub async fn parse_define_module(
		...
		if !self.settings.surrealism_enabled {
			bail!(
				...
				=> "Use of `DEFINE MODULE` is still experimental"
			)
		}
		let kind = if self.eat(t!("IF")) {
			expected!(self, t!("NOT"));
			...
			match self.peek_kind() {
				...
				t!("PASSWORD") => {
					...
					if let PassType::Hash(_) = res.pass_type {
						bail!(
							"Unexpected token `PASSWORD`", @ token.span =>
							"Can't set both a passhash and a password"
						);
					}
				...
				t!("PASSHASH") => {
					...
					if let PassType::Password(_) = res.pass_type {
						bail!(
							"Unexpected token `PASSHASH`", @ token.span =>
							"Can't set both a passhash and a password"
						);
					}
						...
						t!("RECORD") => {
							...
							while self.eat(t!("WITH")) {
								...
								match token.kind {
									...
									_ => unexpected!(self, token, "JWT or REFRESH"),
								}
								self.eat(t!(","));
							}
								...
								t!("RECORD") => {
									match &res.base {
										...
										_ => {
											unexpected!(
												self, peek, "USER", =>
												"`RECORD` bearer can only be defined on a database"
											)
										}
									}
									self.pop_peek();
	...
	pub async fn parse_define_param(&mut self, stk: &mut Stk) -> ParseResult<DefineParamStatement> {
		...
		let name = self.next_token_value::<Param>()?.to_string();
	...
	pub async fn parse_define_event(&mut self, stk: &mut Stk) -> ParseResult<DefineEventStatement> {
			...
			match self.peek_kind() {
				...
				t!("THEN") => {
					...
					if res.then.is_empty() {
						bail!(
							"Expected at least one `THEN` statement", @ token.span =>
				...
				t!("RETRY") => {
					...
					if let EventKind::Async { retry, .. } = &mut res.event_kind {
						*retry = self.next_token_value()?;
						...
						bail!(
							"Unexpected token `RETRY`", @ token.span =>
							"RETRY must be set after ASYNC"
						);
					}
				}
				t!("MAXDEPTH") => {
					...
					if let EventKind::Async { max_depth, .. } = &mut res.event_kind {
						*max_depth = self.next_token_value()?;
						...
						bail!(
							"Unexpected token `MAXDEPTH`", @ token.span =>
							"MAXDEPTH must be set after ASYNC"
						);
					}
				}
				_ => break,
		...
		if res.then.is_empty() {
			bail!(
				"Expected at least one `THEN` statement", @ self.last_span =>
				"`THEN` statement required"
			);
		}
		Ok(res)
		...
		loop {
			match self.peek_kind() {
				...
				t!("FULLTEXT") => {
					...
					res.index = Index::FullText(crate::upstream::sql::index::FullTextParams {
				...
				t!("HNSW") => {
					...
					let m = if let Some((m, span)) = m {
						if m > 127 {
							bail!(
								"Invalid value for HNSW parameter `M`", @ span =>
								"`M` cannot be larger then 127"
							)
						}
						m
					} else {
			...
			(field_span, Index::FullText(_) | Index::Hnsw(_)) => {
				if res.cols.len() != 1 {
					if let Some(field_span) = field_span {
						bail!(
							"Expected one column, found {}", res.cols.len(), @ field_span
						);
					} else {
			...
			match self.peek_kind() {
				...
				t!("TABLES") => {
					...
					match next.kind {
						...
						_ => {
							unexpected!(self, next, "`NONE`, `AUTO`, `INCLUDE` or `EXCLUDE`")
						}
					}
				}
				t!("FUNCTIONS") => {
