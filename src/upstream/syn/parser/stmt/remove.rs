use crate::upstream::sql::statements::remove::{
...
use crate::upstream::sql::{ModuleName, Param};
	...
	pub async fn parse_remove_stmt(&mut self, stk: &mut Stk) -> ParseResult<RemoveStatement> {
		...
		let res = match next.kind {
			...
			t!("MODULE") => {
				if !self.settings.surrealism_enabled {
					bail!(
						...
						.last_span() => "Use of `REMOVE MODULE` is still experimental"
					)
				}
				let if_exists = if self.eat(t!("IF")) {
					expected!(self, t!("EXISTS"));
			...
			t!("TABLE") => {
				...
				let name = stk.run(|stk| self.parse_expr_field(stk)).await?;
				RemoveStatement::Table(crate::upstream::sql::statements::RemoveTableStatement {
					name,
					if_exists,
					expunge,
				})
