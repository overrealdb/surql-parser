...
use crate::upstream::sql::index::{Distance, VectorType};
use crate::upstream::sql::reference::{Reference, ReferenceDeleteStrategy};
...
use crate::upstream::syn::token::{DistanceKind, Span, TokenKind, VectorTypeKind, t};
use reblessive::Stk;
...
impl Parser<'_> {
	...
	pub fn check_idiom(
		...
		match kind {
			MissingKind::Split => {
				bail!(
					"Missing split idiom `{:?}` in statement selection", idiom.to_sql(),
					@ idiom_span, @ field_span => "Idiom missing here",
				)
			}
			MissingKind::Order => {
				bail!(
					"Missing order idiom `{}` in statement selection", idiom.to_sql(), @
					idiom_span, @ field_span => "Idiom missing here",
				)
			}
			MissingKind::Group => {
				bail!(
					"Missing group idiom `{}` in statement selection", idiom.to_sql(), @
					idiom_span, @ field_span => "Idiom missing here",
				)
			}
		};
	...
	pub fn try_parse_group(
		...
		if self.eat(t!("ALL")) {
			if let Some(split_span) = split_span {
				...
				bail!(
					"SPLIT and GROUP are mutually exclusive", @ split_span =>
					"SPLIT cannot be used with GROUP", @ group_span =>
	...
	pub fn parse_base(&mut self) -> ParseResult<Base> {
		...
		match next.kind {
			...
			t!("ROOT") => Ok(Base::Root),
			_ => unexpected!(self, next, "'NAMEPSPACE', 'DATABASE' or 'ROOT'"),
		}
	}
