...
use crate::upstream::syn::lexer::compound::CompoundToken;
...
pub use builtin::{PATHS, PathKind};
...
impl<'a> Parser<'a> {
	...
	fn expect_closing_delimiter(&mut self, kind: TokenKind, should_close: Span) -> ParseResult<()> {
		...
		if peek.kind != kind {
			bail!(
				"Unexpected token `{}` expected delimiter `{kind}`", peek.kind, @ self
				.recent_span(), @ should_close => "expected this delimiter to close"
			);
		}
		self.pop_peek();
		Ok(())
...
impl StatementStream {
	...
	pub fn parse_partial(
		...
		if parser.peek().is_eof() {
			if buffer.len() > u32::MAX as usize {
				let error = syntax_error!(
					"Cannot parse query, statement exceeded maximum size of 4GB", @
					parser.last_span()
				);
				return Err(error
		...
		if !parser.eat(t!(";")) {
			...
			let error = syntax_error!(
				"Unexpected token `{}` expected the query to end.", peek.kind.as_str(), @
		...
		match self.stack.enter(|stk| parser.parse_statement(stk)).finish() {
			Ok(x) => {
				if !parser.peek().is_eof() && !parser.eat(t!(";")) {
					...
					let error = syntax_error!(
						"Unexpected token `{}` expected the query to end.", peek.kind
						.as_str(), @ peek.span =>
