...
use crate::upstream::sql::{Block, Expr, Literal};
use crate::upstream::syn::lexer::compound;
use crate::upstream::syn::parser::mac::expected;
use crate::upstream::syn::parser::{ParseResult, Parser, enter_object_recursion};
...
impl Parser<'_> {
			...
			&& let Some(x) = self
				.speculate(stk, async |stk, this| {
					enter_object_recursion!(
						this = this => { let Ok(key) = this.parse_object_key() else {
						return Ok(None) }; if ! this.eat(t!(":")) { return Ok(None) }
						let value = stk.run(| stk | this.parse_expr_inherit(stk)).
						await ?; let res = vec![ObjectEntry { key, value }]; if this
						.eat(t!(",")) { this.parse_object_inner(stk, start, res).
						await .map(Some) } else { this
						.expect_closing_delimiter(t!("}"), start) ?; Ok(Some(res)) }
						}
					)
				})
				.await?
	...
	pub(super) async fn parse_object(
		...
		enter_object_recursion!(
			this = self => { return this.parse_object_inner(stk, start, Vec::new()).
			await; }
		)
	}
	...
	pub async fn parse_set(&mut self, stk: &mut Stk, start: Span) -> ParseResult<Vec<Expr>> {
		enter_object_recursion!(
			this = self => { return this.parse_set_inner(stk, start). await; }
		)
	}
