...
use crate::upstream::syn::parser::mac::{expected, expected_whitespace};
use crate::upstream::syn::parser::unexpected;
						...
						if self.peek().kind == t!(",") {
							let x = match number {
								Numeric::Duration(_) | Numeric::Decimal(_) => {
									bail!(
										"Unexpected token, expected a non-decimal, non-NaN, number",
										@ number_span =>
										"Coordinate numbers can't be NaN or a decimal"
									);
								}
		...
		let key = match peek.kind {
			...
			t!("+") => {
				...
				let digits_token = if let Some(digits_token) = self.peek_whitespace() {
					...
					bail!(
						"Unexpected whitespace",@ self.last_span() =>
