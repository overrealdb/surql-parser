...
use crate::compat::capabilities::ExperimentalTarget;
use crate::compat::err::Error;
use crate::compat::types::{PublicDatetime, PublicDuration, PublicRecordId, PublicValue};
use crate::config::{MAX_OBJECT_PARSING_DEPTH, MAX_QUERY_PARSING_DEPTH};
use crate::upstream::sql::kind::KindLiteral;
use crate::upstream::sql::{Ast, Block, Expr, Function, Idiom, Kind};
...
pub fn datetime(input: &str) -> Result<PublicDatetime> {
	...
	match Lexer::lex_datetime(input) {
		Ok(x) => Ok(x),
		Err(e) => bail!(Error::InvalidQuery(e.render_on(input))),
	}
}
