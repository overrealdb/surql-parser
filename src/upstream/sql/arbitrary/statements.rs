use crate::upstream::sql::access_type::{BearerAccess, BearerAccessSubject};
use crate::upstream::sql::arbitrary::{
	self, arb_group, arb_opt, arb_order, arb_splits, arb_vec1, atleast_one, insert_data,
};
use crate::upstream::sql::kind::KindLiteral;
use crate::upstream::sql::statements::SetStatement;
use crate::upstream::sql::statements::alter::{
...
impl<'a> Arbitrary<'a> for DefineAccessStatement {
	fn arbitrary(u: &mut arbitrary::Unstructured<'a>) -> arbitrary::Result<Self> {
		...
		let duration = crate::upstream::sql::access::AccessDuration {
			grant: u.arbitrary()?,
			token: u.arbitrary()?,
			session: u.arbitrary()?,
		};
